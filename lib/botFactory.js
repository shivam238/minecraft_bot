...
function createBot(aiManager, lifecycleApi) {
  ...
  bot.on('entityHurt', (entity) => {
    ...
    if (
      ...
      if (ownerSwing) {
        // entity IS the exact object that got hurt — no search needed, always accurate!
        if (bot.pvp.target && bot.pvp.target.id !== entity.id) {
          try { bot.pvp.stop(); } catch (_) {}
        }
        bot.pvp.attack(entity);
        log.info(`Wolf mode: attacking ${entity.username || entity.name || entity.type} (owner hit it)`);
      } else {
