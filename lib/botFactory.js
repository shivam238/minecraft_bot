...
function createBot(aiManager, lifecycleApi) {
  ...
  bot.on('entitySwingArm', (swinger) => {
    ...
    if (dist > 5) return; // too far to matter
  ...
  bot.on('entityHurt', (entity) => {
    ...
    if (candidate.username && configOwners.includes(candidate.username)) {
      ...
      state.panicKnockbackUntil = now + 450; // hold off forward key so knockback can play out (~450ms)
      ...
      // Turn to face away immediately (so the bot visually reacts to the hit),
      // but delay pressing forward ~300ms so server-applied knockback can play out first.
      try {
        ...
        // Don't press forward here — panicKnockbackUntil window in the priority loop handles the delay
      } catch (_) {}
    ...
    // Non-owner / mob hit — retaliate normally
