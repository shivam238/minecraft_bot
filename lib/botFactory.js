...
function createBot(aiManager, lifecycleApi) {
  ...
  bot.on('entitySwingArm', (swinger) => {
    ...
    // Wolf mode: owner swung → move to owner + attack nearest mob there
    ...
    if (
      ...
      // First: try to find a mob visible to the bot near the owner
      const ownerPos = swinger.position;
      let target = bot.nearestEntity(
        (e) =>
          (e.type === 'player' || e.type === 'mob') &&
          e !== bot.entity &&
          !configOwners.includes(e.username) &&
          ownerPos.distanceTo(e.position) < 8
      );
      ...
      if (target) {
        ...
        log.info(`Wolf: attacking ${target.username || target.name || target.type} near owner`);
      } else {
        // Bot too far — walk to owner first, then entityHurt will handle the attack
        const botDist = bot.entity.position.distanceTo(ownerPos);
        log.info(`Wolf: owner swung, no mob visible (bot is ${botDist.toFixed(1)} blocks away — moving to owner)`);
        if (botDist > 4 && bot.pathfinder) {
          try {
            const { goals } = require('mineflayer-pathfinder');
            bot.pathfinder.setGoal(new goals.GoalNear(ownerPos.x, ownerPos.y, ownerPos.z, 2), true);
          } catch (_) {}
        }
      }
    }
  ...
  bot.on('entityHurt', (entity) => {
    ...
    if (
      ...
      if (ownerSwing) {
        // Attack the exact entity the owner hit (by ID) — most accurate
        ...
        if (target) {
          // Always switch to the correct target, even if already attacking something else
          if (bot.pvp.target && bot.pvp.target.id !== target.id) {
            try { bot.pvp.stop(); } catch (_) {}
          }
          bot.pvp.attack(target);
          log.info(`Wolf mode: attacking ${target.username || target.name || target.type} (owner hit it)`);
        }
      } else {
        log.info(`Wolf: entityHurt but no recent owner swing (window=${ATTRIBUTION_WINDOW_MS}ms)`);
      }
      return;
    }
