...
function createBot(aiManager, lifecycleApi) {
  ...
  bot.on('entitySwingArm', (swinger) => {
    ...
    // Just record the swing for entityHurt to use — never attack here (would pick wrong mob)
    const configNow = getConfig();
    const configOwners = configNow.owners || [];
    if (swinger.username && configOwners.includes(swinger.username)) {
      log.info(`Wolf: owner swing recorded — waiting for entityHurt to identify exact target`);
    }
  });
