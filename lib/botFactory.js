    ...
    if (
      ...
      if (target) {
        if (bot.pvp.target !== target) {
          ...
          log.info(`Wolf mode: attacking ${target.username || target.name || target.type}`);
        }
      } else {
        const nearby = Object.values(bot.entities)
          .filter((e) => e !== bot.entity && bot.entity.position.distanceTo(e.position) < 15)
          .map((e) => `${e.name || e.type}(${e.username || ''})`)
          .join(', ');
        log.info(`Wolf: no target. Nearby entities: ${nearby || 'none'}`);
      }
    }
  });
