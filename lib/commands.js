...
async function handleOwnerCommand(username, message, aiManager, lifecycleApi) {
  ...
  switch (command) {
    case 'help':
      bot.chat(
        'Commands: !status !follow(!come) !guard !afk !reset !stop !goto x y z !sleep !wake !drop !store(!chest) !tpa [name] !accept !say [msg] !speed slow|normal|fast|turbo !addowner [name] !removeowner [name] !shutdown'
      );
      return true;
