  ...
  switch (intent) {
    ...
    case 'sleep':
      ...
      walkToBedAndSleep({
        ...
        onNoBed: () => log.fail('AI sleep: no bed found'),
        onNavFail: () => {
...
async function handleOwnerCommand(username, message, aiManager, lifecycleApi) {
  ...
  switch (command) {
    case 'help':
      bot.chat(
        'Commands: !status !follow(!come) !guard !afk !reset !stop !goto x y z !sleep !wake !drop !tpa [name] !accept !say [msg] !speed slow|normal|fast|turbo !addowner [name] !removeowner [name] !shutdown'
      );
      return true;
    ...
    case 'reset':
      ...
      state.botState = 'afk';
      scheduleNextAFK(true);
      bot.chat(`🔄 Reset! AFK mode restarted (speed: ${state.afkSpeedMultiplier}x).`);
      log.ok('AFK reset by owner');
      return true;
...
async function handleChat(username, message, aiManager, lifecycleApi) {
  ...
  if (!message.toLowerCase().includes(botName)) return;
