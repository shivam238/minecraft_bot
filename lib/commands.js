...
async function handleOwnerCommand(username, message, aiManager, lifecycleApi) {
  ...
  switch (command) {
    case 'help':
      bot.chat(
        'Cmds: !status !follow !guard !afk !reset !stop !goto x y z !sleep !wake !drop !store !tpa [n] !accept !say [msg] !speed slow|normal|fast|turbo'
      );
      bot.chat(
        'Toggles: !polite (pvp off/on) | !farming (auto-farm) | !autosleep | !autodefense | !randomchat'
      );
      return true;
    ...
    case 'polite':
      ...
      config.politeMode = state.politeMode;
      saveConfig();
      if (state.politeMode) {
        if (bot.pvp && bot.pvp.target && bot.pvp.target.type === 'player') bot.pvp.stop();
        bot.chat('😇 Polite mode ON — nahi marunga kisi player ko!');
        log.ok('Polite mode enabled');
      } else {
        bot.chat('⚔️ Polite mode OFF — PVP wapas ON!');
        log.ok('Polite mode disabled');
      }
      return true;

    case 'farming': {
      config.farming = !config.farming;
      saveConfig();
      if (config.farming) {
        bot.chat('🌾 Farming ON — kheti shuru karunga thodi der mein!');
        log.ok('Auto-farming enabled');
        // Run first cycle immediately
        runFarmingCycle();
      } else {
        bot.chat('🌾 Farming OFF.');
        log.ok('Auto-farming disabled');
      }
      return true;
    }
    ...
    case 'autosleep': {
      ...
      saveConfig();
      bot.chat(`😴 Auto-sleep: ${config.autoSleep ? 'ON ✅' : 'OFF ❌'}`);
      log.ok(`autoSleep toggled: ${config.autoSleep}`);
      return true;
    }
    ...
    case 'autodefense': {
      ...
      saveConfig();
      bot.chat(`🛡️ Auto-defense: ${config.autoDefense ? 'ON ✅' : 'OFF ❌'}`);
      log.ok(`autoDefense toggled: ${config.autoDefense}`);
      return true;
    }
    ...
    case 'randomchat': {
      ...
      saveConfig();
      bot.chat(`💬 Random chat: ${config.randomChatEnabled ? 'ON ✅' : 'OFF ❌'}`);
      log.ok(`randomChatEnabled toggled: ${config.randomChatEnabled}`);
      return true;
    }
    ...
    case 'reset':
      ...
      config.politeMode = false;
      config.farming = false;
      saveConfig();
      state.botState = 'afk';
      scheduleNextAFK(true);
      bot.chat('🔄 Reset! AFK mode, polite cleared, farming off.');
      log.ok('Full reset by owner');
      return true;
