...
function executePriorityLogic(priority) {
  ...
  switch (priority) {
    ...
    case Priorities.PANICKING: {
      ...
      if (isDirectionSafe(fleeYaw, WALK_REACH)) runYaw = fleeYaw;
      // 2. Straight away from attacker
      if (runYaw === null && isDirectionSafe(straightYaw, WALK_REACH)) runYaw = straightYaw;
      ...
      if (runYaw === null) {
        for (let i = 1; i <= 4; i++) {
          ...
          for (const sign of [1, -1]) {
            ...
            if (isDirectionSafe(tryYaw, WALK_REACH)) { runYaw = tryYaw; break; }
          }
