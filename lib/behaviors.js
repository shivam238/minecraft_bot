...
function checkAutoSleep() {
  ...
  // Only sleep if at least one other player in the server is already sleeping
  // (This way bot doesn't sleep solo mid-fight, and helps skip night when others want to)
  const otherPlayers = Object.values(bot.players).filter(
    (p) => p.username !== bot.username && p.entity
  );
  const someoneSleeping = otherPlayers.some((p) => p.entity && p.entity.metadata &&
    // sleeping metadata flag (pose === 6 in 1.17+ = sleeping)
    (p.entity.pose === 6 || p.entity.sleeping === true)
  );

  // If there are other players but none are sleeping — don't sleep yet
  if (otherPlayers.length > 0 && !someoneSleeping) return;

  const bed = findNearbyBed(40);
  if (!bed) return;

  log.info('Auto-sleep: others sleeping — heading to bed...');
  stopCurrentTasks();
