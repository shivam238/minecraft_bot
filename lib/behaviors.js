...
async function runWalkAction() {
  ...
  for (let attempt = 0; attempt < 15; attempt++) {
    // Dynamically scale down distance as attempts fail to find safe paths closer to the bot
...
function executePriorityLogic(priority) {
  ...
  switch (priority) {
    ...
    case Priorities.PANICKING: {
      ...
      // Pick best runnable yaw: zigzag first, then straight away, then any safe direction
      let runYaw = null;
      if (isDirectionSafe(fleeYaw, 1.5)) {
        runYaw = fleeYaw;
      } else if (isDirectionSafe(straightYaw, 1.5)) {
        runYaw = straightYaw;
      } else {
        // Try 8 angles fanning out from straight-away until one is safe
        for (let i = 1; i <= 4; i++) {
          const spread = (Math.PI / 4) * i;
          if (isDirectionSafe(straightYaw + spread, 1.5)) { runYaw = straightYaw + spread; break; }
          if (isDirectionSafe(straightYaw - spread, 1.5)) { runYaw = straightYaw - spread; break; }
        }
      }

      if (runYaw !== null) {
        // Instant face + sprint — no smooth look, passive mobs turn and run immediately
        bot.look(runYaw, 0, true);
        bot.setControlState('sneak', false);
        bot.setControlState('forward', true);
        bot.setControlState('sprint', true);
      } else {
        // Nowhere safe to run — cower in place (sneak) like a cornered animal
        releaseMovementKeys();
        bot.setControlState('sneak', true);
      }
      break;
