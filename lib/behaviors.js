...
const { clearSleepListener, wakeUp, findNearbyBed, walkToBedAndSleep } = require('./sleep');
...
async function runDirectWalk() {
  ...
  // Only jump if still safe — never jump on a void edge
  if (isAFKActive() && isCurrentGroundSafe() && isDirectionSafe(bot.entity.yaw, 1.5) && Math.random() < 0.2) {
...
async function runJumpWalk() {
  ...
  // Jump-walking is dangerous near edges — require a longer clear runway than sprint
  if (!isDirectionSafe(targetYaw, 4)) return;

  await smoothLook(targetYaw, -0.1, 3, 25);
  if (!isAFKActive() || !isCurrentGroundSafe() || !isDirectionSafe(targetYaw, 3)) return;
  ...
  for (let i = 0; i < jumps; i++) {
    if (!isAFKActive() || !isCurrentGroundSafe() || !isDirectionSafe(bot.entity.yaw, 2)) break;
    bot.setControlState('jump', true);
...
function pickAFKActionType(forceWalk) {
  ...
  const types = ['look', 'idle_look', 'look_swing', 'look_block', 'crouch_peek', 'jump_walk', 'sprint_burst', 'direct_walk', 'walk'];
  const thresholds = [0.02, 0.05, 0.08, 0.11, 0.15, 0.25, 0.40, 0.65, 1.0];
...
function handlePriorityTransition(oldPriority, newPriority) {
  ...
  if (bot.pathfinder && newPriority !== Priorities.SLEEPING) bot.pathfinder.stop();
  ...
  switch (priority) {
    case Priorities.VOID_ESCAPE: {
      ...
      if (now - state.lastEmergencyActionTime > 2000) {
        const escVel = bot.entity.velocity;
        const isAirborne = Math.abs(escVel.y) > 0.3;

        if (isAirborne) {
          // Still falling — can't pathfind mid-air, just hold still and wait to land.
          releaseMovementKeys();
        } else if (!bot.pathfinder.isMoving()) {
          // Landed and not already navigating — find closest safe position.
          releaseMovementKeys();
        ...
        // If pathfinder IS already moving: leave it alone — it is navigating to safety.
    ...
    case Priorities.EMERGENCY_SURVIVAL: {
      const now = Date.now();
      // Pathfinder is already navigating to safety — don't interrupt it.
      // Without this guard the 2 s timer re-issues the same goal and, worse,
      // the early-stop used to kill it every 200 ms creating an infinite loop.
    ...
    case Priorities.PANICKING: {
      ...
      // SPRINT needs 4-block clear runway (sprint speed ~5.6 m/s, tick=200ms → >1m per tick)
      // WALK  needs 2-block clear runway as fallback
      const SPRINT_REACH = 4.0;
      ...
      if (isDirectionSafe(fleeYaw, SPRINT_REACH)) {
        runYaw = fleeYaw; canSprint = true;
      } else if (isDirectionSafe(fleeYaw, WALK_REACH)) {
        runYaw = fleeYaw; canSprint = false;
      }
      // 2. Straight away from attacker
      if (runYaw === null) {
        if (isDirectionSafe(straightYaw, SPRINT_REACH)) {
          runYaw = straightYaw; canSprint = true;
        } else if (isDirectionSafe(straightYaw, WALK_REACH)) {
          runYaw = straightYaw; canSprint = false;
        }
      }
      // 3. Fan out ±45°/90°/135°/180° until a safe direction is found
      if (runYaw === null) {
        for (let i = 1; i <= 4; i++) {
          const spread = (Math.PI / 4) * i;
          for (const sign of [1, -1]) {
            const tryYaw = straightYaw + spread * sign;
            if (isDirectionSafe(tryYaw, SPRINT_REACH)) {
              runYaw = tryYaw; canSprint = true; break;
            } else if (isDirectionSafe(tryYaw, WALK_REACH)) {
              runYaw = tryYaw; canSprint = false; break;
            }
          }
      ...
      if (runYaw !== null) {
        ...
        // Keep sneak ON while moving — Minecraft sneak prevents falling off edges
        bot.setControlState('sneak', true);
        bot.setControlState('forward', true);
        bot.setControlState('sprint', false); // never sprint during panic; sneak+sprint conflict causes edge falls
      } else {
        // No safe raw-key direction — switch to pathfinder to reach a known safe spot.
        // This is smarter than freezing when cornered near a void edge:
        // pathfinder can find a route across the platform even if direct flee is blocked.
        releaseMovementKeys();
        if (!bot.pathfinder.isMoving()) {
          const safePos = findClosestSafePosition();
          if (safePos) {
            ...
            // No known safe position — sneak in place as last resort
            bot.setControlState('sneak', true);
          }
        }
      }
    ...
    case Priorities.FOLLOWING: {
      ...
      if (!targetPlayer) {
        ...
        if (dist > maxDist) {
          bot.chat(`⚠️ You are too far (>${maxDist} blocks)! Reverting to AFK.`);
          stopCurrentTasks();
          state.botState = 'afk';
          scheduleNextAFK(true);
        } else {
 ...
 * Only runs when autoSleep is enabled in config and bot is in AFK/idle state.
 */
function checkAutoSleep() {
  ...
  log.info('Auto-sleep: nighttime, heading to bed...');
  stopCurrentTasks();
  state.botState = 'sleeping';
  // Set cooldown so we don't retry in a tight loop if something goes wrong
...
function runPriorityManagerTick() {
  ...
  // Guard: isCurrentGroundSafe() returns false during normal jumps (feet are airborne).
  // Only treat ground as unsafe when the bot is grounded (not moving up).
  const isJumpingUp = vel.y > 0.1;
  const standingOnUnsafe = !isJumpingUp && !isCurrentGroundSafe();
  ...
  // Immediate interception — split into "truly dangerous" vs "mildly unsafe":
  //   Truly dangerous (lava / actual void fall): always stop immediately.
  //   Mildly unsafe (standing on edge tile): stop ONLY if not already in an escape
  //   priority — otherwise we create a destroy-rebuild loop where the safety check
  //   kills the escape pathfinder every 200ms and the bot never actually moves.
  const trulyDangerous = standingInLiquid || fallingIntoVoid;
  const currentlyEscaping = state.currentPriority === Priorities.VOID_ESCAPE ||
                             state.currentPriority === Priorities.EMERGENCY_SURVIVAL;

  if (trulyDangerous) {
    if (bot.pathfinder.isMoving()) {
      log.warn(fallingIntoVoid ? 'Void fall detected — emergency stop' : 'Lava — emergency stop');
      try { bot.pathfinder.stop(); } catch (_) {}
    }
    releaseMovementKeys();
  } else if (standingOnUnsafe && !currentlyEscaping) {
    // Mildly unsafe but not in escape mode — stop so priority system can respond.
    if (bot.pathfinder.isMoving()) {
      log.warn('Unsafe ground — stopping pathfinder');
      try { bot.pathfinder.stop(); } catch (_) {}
    }
    releaseMovementKeys();
  }
  // If mildlyUnsafe AND currentlyEscaping: do nothing — let escape pathfinding continue.
