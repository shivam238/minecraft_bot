...
function checkAutoSleep() {
  ...
  // (Vanilla Minecraft requires everyone to sleep for night to skip)
  const otherPlayers = Object.values(bot.players).filter(
    (p) => p.username !== bot.username && p.entity
  );
  const everyoneSleeping = otherPlayers.length > 0 && otherPlayers.every((p) =>
    p.entity && (p.entity.pose === 6 || p.entity.sleeping === true)
  );

  // If there are other players and NOT all are sleeping — wait
  if (otherPlayers.length > 0 && !everyoneSleeping) return;
