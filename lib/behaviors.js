...
const { runFarmingCycle } = require('./farming');
...
async function runDirectWalk() {
  ...
  for (const offset of shuffled) {
    ...
    if (isDirectionSafe(candidateYaw, 2.5) && isDirectionFarmSafe(candidateYaw, 2.5)) {
  ...
  if (!isAFKActive() || !isCurrentGroundSafe()
    || !isDirectionSafe(bot.entity.yaw, 2)
    || !isDirectionFarmSafe(bot.entity.yaw, 2)) {
    releaseMovementKeys();
    return;
  }
...
async function runSprintBurst() {
  ...
  // Check void safety AND crop safety before sprinting
  if (!isDirectionSafe(bot.entity.yaw, 3)) return;
  if (!isDirectionFarmSafe(bot.entity.yaw, 3)) return;
  ...
  // Re-check after look adjustment (yaw changed slightly)
  if (!isAFKActive() || !isCurrentGroundSafe()
    || !isDirectionSafe(bot.entity.yaw, 2)
    || !isDirectionFarmSafe(bot.entity.yaw, 2)) {
    releaseMovementKeys();
...
// Sprint-jump — 1 jump while moving forward (parkour feel)
// Safety: checks void AND crop blocks ahead; cancels momentum with back-key if abort needed
async function runJumpWalk() {
  ...
  // Need 8-block void-safe runway AND no crops in next 5 blocks
  if (!isDirectionSafe(targetYaw, 8)) return;
  if (!isDirectionFarmSafe(targetYaw, 5)) return;

  await smoothLook(targetYaw, -0.1, 3, 25);
  // Re-verify after look
  if (!isAFKActive() || !isCurrentGroundSafe() || isOnSensitiveGround()
    || !isDirectionSafe(bot.entity.yaw, 8)
    || !isDirectionFarmSafe(bot.entity.yaw, 5)) return;

  // Helper: release all keys AND briefly press back to kill sprint momentum
  ...
  // Only 1 jump — enough for natural parkour feel without over-committing
  // Check 6 blocks ahead right before the jump (we've already moved ~0.2s forward)
  if (!isAFKActive() || !isCurrentGroundSafe()
    || !isDirectionSafe(bot.entity.yaw, 6)
    || !isDirectionFarmSafe(bot.entity.yaw, 4)
    || isOnSensitiveGround()) {
    await safeAbort();
    return;
  }
  ...
  bot.setControlState('jump', false);
  // Let bot land, then hard-stop
  await new Promise((r) => setTimeout(r, 400));

  // Post-jump: check we haven't drifted onto crops or edge
  if (!isAFKActive() || !isCurrentGroundSafe()
    || !isDirectionSafe(bot.entity.yaw, 4)
    || isOnSensitiveGround()) {
    await safeAbort();
    return;
  }
...
function startSmartLoops() {
  ...
  // Farming loop — runs every 3 minutes, no-ops when disabled
  state.farmingInterval = setInterval(() => runFarmingCycle(), 3 * 60 * 1000);
