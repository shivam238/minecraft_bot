...
function executePriorityLogic(priority) {
  ...
  switch (priority) {
    ...
    case Priorities.PANICKING: {
      ...
      // Update attacker's position each tick if they're still nearby (keeps flee direction fresh)
      ...
      // Send additional scared messages occasionally during panic
      if (now - state.lastPanicChatTime > 2500 + Math.random() * 1500) {
        const mid = ['bhai pleaseee!!', 'ow!!', 'stop stop STOP!!', 'kyu maar rha hai!!',
                     'AHHH!!', 'bach ke!!', 'nooo!!', '😭😭'];
        bot.chat(mid[Math.floor(Math.random() * mid.length)]);
        state.lastPanicChatTime = now;
      }

      // Run away every 1–1.5 seconds in a new panicked direction
      if (now - state.lastPanicActionTime > 1000 + Math.random() * 500) {
        ...
        // Add zig-zag randomness — perpendicular component like a panicking villager
        const zigzag = (Math.random() - 0.5) * 2.2;
        const tx = dx + (-dz * zigzag);
        const tz = dz + (dx * zigzag);
        const runDist = 4 + Math.random() * 5;

        const fleeYaw = Math.atan2(-tx, -tz);
        const targetPos = bot.entity.position.offset(tx * runDist, 0, tz * runDist);

        if (isDirectionSafe(fleeYaw, 3)) {
          if (bot.pathfinder.isMoving()) {
            try { bot.pathfinder.stop(); } catch (_) {}
          }
          safeSetGoal(new goals.GoalNear(targetPos.x, targetPos.y, targetPos.z, 0.5));
