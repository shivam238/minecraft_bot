...
async function runPickupItems() {
  ...
  log.info(`Pickup: spotted ${item.metadata?.[8]?.itemId || 'item'} at ${Math.round(item.position.x)},${Math.round(item.position.y)},${Math.round(item.position.z)}`);
  safeSetGoal(new goals.GoalNear(item.position.x, item.position.y, item.position.z, 1));
